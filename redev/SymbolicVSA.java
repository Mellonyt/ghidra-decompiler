...
class VSADataTypeManager {
     ...
     */
    private DataType genVSADataStruct(List<Long> struct_layout) {
        ...
        DataTypeManager dataMgr = m_program.getDataTypeManager();
        DataType dt = dataMgr.getDataType(struct_name);
        ...
        dataMgr.addDataType(st, null);

        return st;
    }

    private DataType getVSADataStruct(List<Long> struct_layout) {
...
interface VSAParameterAnalyzer {
    public List<String> inferParameterInUser(Map<Long, Map<String, Set<String>>> symbolic_memory_access_table);

    private void updateOnSystemVLinux64(Function function);
}
...
class SystemVLinux64Parameter implements VSAParameterAnalyzer {
     ...
     */
    public List<String> inferActiveParameters(Map<Long, Map<String, Set<String>>> symbolic_memory_access_table) {
    ...
    private Variable genGPRParameter(String reg_name, int ordinal) {
        String param_name = "GP" + String.valueOf(ordinal);
        String this_scope = "V" + reg_name;
        DataType dt = m_mapScopeDT.get(this_scope);
    ...
    private Variable genFPRParameter(String reg_name, int ordinal) {
    ...
    private void updateOnSystemVLinux64(Function function) {
        ...
        try {
            /* for general purpose registers */
            String[] regGNames = new String[] { "RDI", "RSI", "RDX", "RCX", "R8", "R9" };
            for (int i = 0; i < regGNames.length; i++) {
                Variable p = genGPRParameter(regGNames[i], ordinal);
                if (p != null) {
                    params.add(p);
                    ordinal++;
                } else {
     ...
     */
    public boolean updateParameters(Function function, Map<String, List<Long>> possilbe_struct_scope) {
        m_setAllScopes = new HashSet<>();
        m_mapScopeDT = new HashMap<>();

        for (Map.Entry<String, List<Long>> entmapStruct : possilbe_struct_scope.entrySet()) {
            String scope = entmapStruct.getKey();
            m_setAllScopes.add(scope);

            List<Long> st = entmapStruct.getValue();
            DataType dt = getVSADataStruct(st);
            if (dt != null) {
                m_mapScopeDT.put(scope, dt);
            }
        }

        if (m_setAllScopes.size() > 0 && m_mapScopeDT.size() > 0) {
            System.out.println("Fix parameters");
            updateOnSystemVLinux64(function);
            return true;
        } else {
            return false;
        }
...
public class SymbolicVSA extends GhidraScript {
    ...
    public void run() {
        ...
        while (iter.hasNext() && !monitor.isCancelled()) {
            ...
            // println("385: Find scopes: " + smart.toString());
            ...
            for (String msg : info) {
                println("394:  Find array: " + msg);
            }

            /* Update function parameters in terms of VSA result */
            Map<String, List<Long>> mapStruct = infer.inferStruct(scopeAccess);
            // println("358: Possible structs: " + mapStruct.toString());

            paramUpdater.updateFunctionParameters(f, mapStruct);
        }
    }
...
/*----------------------------copy from DataStructInfer.java-------------------------------------------------------------------*/
class DataStructInfer {
    private SymbolicCalculator m_calc;

    public DataStructInfer() {
        m_calc = SymbolicCalculator.getCalculator();
    }

    private void _findPossibleArrayAccess(Map<Long, Map<String, List<Long>>> array_access, Long line_no, Map<String, Set<String>> value_sets) {
        /*
        ...
        Pair<String, List<Long>> pairAccess = new Pair<>(scope, listVS);
        return pairAccess;
    }

    private void _findPossibleStructAccess(Map<String, Set<String>> mapVS) {
        /* memory accessed by this function */
        List<Long> addrSet;
        String scope;
     ...
     * @return
     */
    public Set<Map<String, List<Long>>> inferMemScopes(
            Map<Long, Map<String, Set<String>>> symbolic_memory_access_table) {
        Map<Long, Map<String, List<Long>>> mapArrayAccess = new HashMap<>();
        ...
        for (Map.Entry<Long, Map<String, Set<String>>> entMapSMAT : symbolic_memory_access_table.entrySet()) {
            ...
            if (mapVS.size() > 4) {
                _findPossibleArrayAccess(mapArrayAccess, line, mapVS);
            } else {
                _findPossibleStructAccess(mapScopeAccess, mapVS);
            }
        }
     ...
     * @return
     */
    public Set<String> inferArrays(Set<Map<String, List<Long>>> possible_array_scope,
     ...
     * @return
     */
    public Map<String, List<Long>> inferStructs(Map<String, List<Long>> all_memory_scopes) {
